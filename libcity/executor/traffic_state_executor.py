...
class TrafficStateExecutor(AbstractExecutor):
    def __init__(self, config, model, data_feature):
        ...
        self.cache_model_file_dir = './libcity/cache/{}/model'.format(self.exp_id)
        self.model_file_dir = './libcity/model/mixer/'
        self.config_file = self.config.get('config_file', None)

        ensure_dir(self.cache_dir)
        ensure_dir(self.evaluate_res_dir)
        ensure_dir(self.summary_writer_dir)
        ensure_dir(self.cache_model_file_dir)
    ...
    def save_model_file(self):
        for root, _, files in os.walk(self.model_file_dir):
            for file in files:
                if file.endswith('.py'):
                    # 构建源文件的完整路径
                    source_file_path = os.path.join(root, file)
                    # 构建目标文件的完整路径
                    target_file_path = os.path.join(self.cache_model_file_dir, file)
                    # 复制文件到目标文件夹
                    shutil.copy2(source_file_path, target_file_path)
        if self.config_file is not None:
            source_file_path = os.path.join('./', self.config_file + '.json')
            # 构建目标文件的完整路径
            target_file_path = os.path.join(self.cache_model_file_dir, self.config_file + '.json')
            # 复制文件到目标文件夹
            shutil.copy2(source_file_path, target_file_path)

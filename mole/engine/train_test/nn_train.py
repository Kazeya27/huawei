...
def nn_train(model, model_path, result_path, config,
             get_loader, loss_fun, get_metric, is_inverse,
             device, n_worker):
    model.to(device)
    model.train()

    loader_train, dataset_train = get_loader(
        'train', config, n_worker)
    loader_valid, dataset_valid = get_loader(
        'valid', config, n_worker)
    ...
    if (os.path.isfile(model_path_final) and
            os.path.isfile(result_path_final)):
        return
    ...
    for i in range(start_epoch, n_epoch):
        ...
        if os.path.isfile(model_path_i):
            ...
            optimizer.load_state_dict(
                model_pkl['optimizer_state_dict'])
            ...
            if os.path.isfile(result_path_i):
                ...
                metric_valid = nn_test(
                    loader_valid, dataset_valid, model,
            ...
            if early_stopper.early_stop or i == n_epoch - 1:
                torch.save(early_stopper.model_pkl,
                           model_path_final)
                joblib.dump(early_stopper.result_pkl,
                            result_path_final)
        ...
        loss_valid = nn_test(
            loader_valid, dataset_valid, model,
            get_metric, device, is_inverse, True)
        metric_valid = nn_test(
            loader_valid, dataset_valid, model,
            get_metric, device, is_inverse, False)
        metric_test = nn_test(

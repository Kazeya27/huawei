...
class TrafficStateDataset(AbstractDataset):
    ...
    def __init__(self, config):
        ...
        # self.parameters_str = \
        #     str(self.dataset) + '_' + str(self.input_window) + '_' + str(self.output_window) + '_' \
        #     + str(self.train_rate) + '_' + str(self.eval_rate) + '_' + str(self.pad_with_last_sample) \
        #         + '_' + str(self.shuffle)
        self.parameters_str = \
            str(self.dataset) + '_' + str(self.input_window) + '_' + str(self.output_window) + '_' \
            + str(self.train_rate) + '_' + str(self.eval_rate) + '_' + str(self.scaler_type) + '_' \
            + str(self.load_external) + '_' + str(self.add_time_in_day) + '_' \
            + str(self.add_day_in_week) + '_' + str(self.pad_with_last_sample) + "_2_4_True"
        self.cache_file_name = os.path.join('./libcity/cache/dataset_cache/',
                                            'traffic_state_{}.npz'.format(self.parameters_str))
        # self.cache_file_name = os.path.join('../pool_mixer/libcity/cache/dataset_cache',
        #                                     'point_based_sd_12_12_0.6_0.2_standard_True_True_True_True_2_4_True.npz')
        ...
        if self.dataset == '171441':
            self.poi_emb = np.load(self.data_path + 'poi_emb.npy')
    ...
    def _load_grid_geo(self):
        ...
        """
        if self.dataset == '171441':
            self.geo_ids = [i for i in range(900)]
            self.len_row = 30
            self.len_column = 30
            self.num_nodes = len(self.geo_ids)
            self._logger.info("加载武汉数据集geo")
            return
        geofile = pd.read_csv(self.data_path + self.geo_file + '.geo')
        self.geo_ids = list(geofile['geo_id'])
        self.num_nodes = len(self.geo_ids)
    ...
    def _load_grid_3d(self, filename):
        ...
        # 求时间序列
        if self.dataset == '171441':
            self.timesolts = list(gridfile['time'][:int(gridfile.shape[0] / (28 * 23))])
        else:
        ...
        len_time = len(self.timesolts)
        if self.dataset == '171441':
            data = np.load(self.data_path + 'data.npz')['data']
            data = data[:, :, :, np.newaxis]
            self._logger.info("加载武汉数据集：" + ': shape=' + str(data.shape) + ", : len_time=" + str(len_time))
            # raise ValueError(self.timesolts[(20962 + 2995 + 5):(20962 + 2995 + 7)])
            # return data[:, :30, :30, :].reshape(data.shape[0], 900, 1)
            return data[:, 50:80, 50:80, :].reshape(data.shape[0], 900, 1)
        data = []
        for i in range(0, df.shape[0], len_time):
            data.append(df[i:i + len_time].values)
        data = np.array(data, dtype=np.float32)  # (len(self.geo_ids), len_time, feature_dim)
    ...
    def visualize(self, data, idx, right=None):
        T, N, D = data.shape
        if right is None:
            right = T

        save_path = f"./pic/test/"
        start_date = datetime.datetime.strptime("01-01 00:00", "%m-%d %H:%M")
        dates = [start_date + timedelta(minutes=i * 30) for i in range(right)]
        os.makedirs(save_path, exist_ok=True)
        os.makedirs(f"./pic/test/", exist_ok=True)

        plt.figure(figsize=(12, 4))
        plt.plot(dates, data[:right, idx, 0])
        # 设置时间刻度间隔为 1 小时
        locator = mdates.HourLocator(byhour=range(0, 24), interval=6)
        formatter = mdates.DateFormatter('%m-%d %H:%M')
        plt.gca().xaxis.set_major_locator(locator)
        plt.gca().xaxis.set_major_formatter(formatter)
        plt.xlim(dates[0], dates[-1] + timedelta(minutes=150))
        # 自动调整时间标签的显示角度
        plt.gcf().autofmt_xdate()
        # plt.xticks(dates, [time.strftime("%m-%d %H:%M") for time in dates], rotation=45)
        plt.ylabel("flow")
        plt.tight_layout()
        plt.show()
        plt.savefig(os.path.join(save_path, self.dataset + ".svg"))  # 可根据需要更改文件名
        plt.close()
        set_trace()
    ...
    def _generate_data(self):
        ...
        # x_list.append(x)
        # y_list.append(y)
        # x = np.concatenate(x_list, dtype=np.float32)
        # y = np.concatenate(y_list, dtype=np.float32)
        # del x_list
        # del y_list
    ...
    def _split_train_val_test(self, x, y):
        ...
        if self.shuffle:
            ...
            x_test = np.zeros(
                ((num_test - self.input_window - self.output_window) * 4, self.input_window, x.shape[2], x.shape[3]),
                dtype=np.float32)
            y_test = np.zeros(
                ((num_test - self.input_window - self.output_window) * 4, self.output_window, y.shape[2], y.shape[3]),
                dtype=np.float32)
            st = 0
            for i in range(4):
    ...
    def _get_scalar(self, scaler_type, x_train, y_train):
        ...
        if scaler_type == "normal":
            ...
            scaler = StandardScaler(mean=x_train, std=y_train)

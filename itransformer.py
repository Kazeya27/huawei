...
class iTransformer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.d_ff = int(config.get("d_ff", 512))
        # Embedding
        self.enc_embedding = DataEmbedding_inverted(self.seq_len, self.d_model, self.embed, 'h',
                                                    self.dropout)
    ...
    def forward(self, batch):
        ...
        x_mark = batch[:, 0, :, 1:]
        dec_out, attns = self.forecast(x, x_mark)
        out = dec_out[:, -self.pred_len:, :].permute(0, 2, 1)
        if self.output_attention:
            return out, attns
        else:
            return out  # [B, L, D]
